...
import { AIMSClient, AIMSClientInstance } from '../src/index';
...
describe('AIMS Client Test Suite:', () => {
  ...
  describe("when instantiating a client", () => {
    it("should capture the specified client", () => {
      let clientInstance = new AIMSClientInstance( ALClient );
      expect( clientInstance['client'] ).to.equal( ALClient );
    } );
  } );
  ...
  describe('when retrieving all access keys for a user', () => {
    it('should call fetch() on the ALClient instance to the access_keys endpoint for the supplied user id value', async() => {
      let result = await AIMSClient.getAccessKeys(accountId, userId );
      expect(stub.callCount).to.equal(1);
      const payload = stub.args[0][0];
      expect( payload.method ).to.equal( "GET" );
